...
- Tutorial 5's clips now push in on the explosion instead of showing it small and off-centre in the corner of the editor — each clip opens on the full app, glides in until the effect fills the frame with the subject locked dead centre, and eases back out so the loop closes seamlessly; the interaction clips do the same per step, pushing in on the emitter tree for the drag, the curve editor for the Index staircase, and the Atlas Frame Picker as it highlights Frame 0 and Frame 15
