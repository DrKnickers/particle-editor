...
describe("CurveEditorPanel — key copy/cut/paste", () => {
  ...
  // (2026-07 audit C-006). The per-key fan-out it replaced captured a separate
  ...
  // 2026-07 audit C-006. The discriminating case: with the old per-key fan-out
  ...
  it("Ctrl+V of a multi-key clipboard issues exactly ONE batched request (C-006)", async () => {
    const { bridge } = makeStubBridgeMultiInterior(0);
    await focusScale(bridge);
    clickKeyByTime(25);
    clickKeyByTime(50, true);
...
// 2026-07 audit C-005. Four mutation completions write SELECTION state
...
describe("CurveEditorPanel — late mutation completions are emitter-scoped (C-005)", () => {
