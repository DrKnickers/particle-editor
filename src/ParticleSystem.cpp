...
void ParticleSystem::Emitter::readTracks(ChunkReader& reader)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		while ((type = reader.nextMini()) == 5)
		{
			...
			// Aggregate cap (2026-07 audit, B-3). Every key above is individually
    ...
    try
    {
	    ...
	    while (type != -1)
	    {
	        ...
	        else if (type == 0x0003)
	        {
	            ...
	            // Aggregate cap (2026-07 audit, B-4). `remaining` bounds the BYTES,
...
void ParticleSystem::ValidateEmitterGraph()
{
    ...
    // and not merely surviving (2026-07 audit, P2-04). The DFS stack size IS the
