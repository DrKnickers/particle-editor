...
// above (2026-07 audit, B-1). Depth alone leaves a shallow-but-enormous document
...
// Total ATTRIBUTE PAIRS for one document (2026-07 audit, B-UX-4). A shallow
...
// (2026-07 audit, B-UX-5). This counts temporary token-vector pushes, roster /
...
// (2026-07 audit, P2-04). kMaxAloEmitters bounds how MANY emitters a file may
...
//
// Top-level 0x0400 mesh containers in one .alo (audit B-2). Each is an
...
// Material containers across ALL meshes in one model (audit B-UX-3a). Each
...
// B-UX-3b). Count raw 0x10102..0x10106 leaves, including duplicate names.
static const unsigned long kMaxAloShaderParamsTotal = 32768u;
// Keys in ONE emitter track (audit B-3). Track::KeyMap is a std::multiset, so
...
// Link-exempt entries in one 0x0003 chunk (audit B-4). The count was bounded
...
// Roster names taken from one GameObjectList.lua (audit B-7). The reader already
...
// Inbound WebMessage cap, in UTF-16 CHARACTERS (2026-07 audit, P2-02). Bridge
