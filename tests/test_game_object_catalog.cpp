...
#include "managers.h"
#include "files.h"
#include "ResourceLimits.h"   // kMaxRosterEntries / kMaxXmlFileBytes (B-7 roster cap)
...
// Literal B-UX-5 accounting fixture, deliberately independent of the production
// constant:
...
int main(int argc, char** argv)
{
    ...
    // ---- B-UX-5: aggregate fieldable-token processing budget --------------
    //
    ...
    // numeric token, so omitting any individual charge wrongly accepts it.
    {
        ...
        CHECK(exactXml.size() < kMaxXmlFileBytes,
              "B-UX-5: literal 131072 fixture is below the XML byte cap");
        ...
        CHECK(exactOk == true,
              "B-UX-5: exactly 131072 fieldable-token work operations build");
        CHECK(exactCatalog.objects.size() == 1u,
              "B-UX-5: exact boundary publishes the one expected model object");
        ...
        CHECK(exactMember && exactMember->fieldable
              ...
              "B-UX-5: exact-boundary member expansion publishes the exact field-source value");
    }

    {
        ...
        CHECK(overXml.size() < kMaxXmlFileBytes,
              "B-UX-5: literal 131073 fixture is below the XML byte cap");
        ...
        CHECK(overOk == false,
              "B-UX-5: literal 131073 returns false (specific refusal value)");
        CHECK(overCatalog.objects.empty(),
              "B-UX-5: rejected 131073 build publishes no partial objects");
        CHECK(overCatalog.hardpoints.empty(),
              "B-UX-5: rejected 131073 build publishes no partial hardpoints");
    }

    // ---- B-7: GameObjectList.lua roster fan-out is COUNT-capped, not just
    // byte-capped ----
    //
    ...
    // invisible from outside, and counting distinct names would be the B-6
    ...
    // marked FS_Roster. That is a property the cap alone decides.
    {
        ...
        CHECK(lua.size() < kMaxXmlFileBytes,
              "B-7 fixture is under the byte cap (so the COUNT cap is what's under test)");
        rfm.files["Data\\Scripts\\Library\\GameObjectList.lua"] = lua;

        GameObjectCatalog rc;
        CHECK(BuildGameObjectCatalog(rfm, rc), "B-7: oversized-roster catalog builds");
        ...
        CHECK(late && (late->fieldSource & FS_Roster) == 0,
              "B-7: roster name PAST the cap is not applied (cap holds)");
        ...
        CHECK(early && (early->fieldSource & FS_Roster) != 0,
              "B-7: roster name BEFORE the cap is still applied (overreach guard)");
    }
    ...
    // largest observed real roster, so nothing legitimate may come near it.
    {
        ...
        CHECK(BuildGameObjectCatalog(nfm, nc), "B-7: normal-roster catalog builds");
        const GameObjectRef* last = find(nc, "Last_Trooper");
        CHECK(last && (last->fieldSource & FS_Roster) != 0,
              "B-7: a ~900-entry roster's LAST name still applies (real content untouched)");
    }
