...
REM x64 console exe. SpawnSchedule.h is header-only (std only); the test also
REM source-checks the production binding because no production TU is linked.
setlocal
...
cl /nologo /EHsc /std:c++17 /MDd /Zi ^
   ...
   "%ROOT%\tests\test_spawn_schedule.cpp" ^
   /link kernel32.lib
