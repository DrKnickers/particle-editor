@echo off
REM Build the WebView2 init modal policy regression test as a standalone
REM x64 console exe. WebViewModalPolicy.h is header-only.
setlocal
...
cl /nologo /EHsc /std:c++17 /MDd /Zi ^
   ...
   /Fe:tests\test_webview_modal_policy.exe /Fo:tests\obj\ ^
   tests\test_webview_modal_policy.cpp ^
