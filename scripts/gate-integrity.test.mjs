...
test("parseSkippedCount reads `node --test`'s reversed word order (the scripts lane was always 0)", () => {
  ...
  // audit G-11 never fired for the one lane it was written for.
...
test("recordSmokeVerdict FAILS a recorder that wrote frames and then crashed (the G-6 false green)", () => {
...
// ── skip-as-pass: internal skips (audit G-5 + G-11) ───────────────────────
...
test("skipBudgetVerdict FAILS a lane that skipped more tests than it declared (the G-11 false green)", () => {
...
test("selfSkipVerdict FAILS a binary that printed SKIP: and exited 0 (the G-5 false green)", () => {
...
test("selfSkipVerdict downgrades to a VISIBLE skip under --allow-missing-capabilities", () => {
  ...
  // restores exactly the silence G-5 is about.
